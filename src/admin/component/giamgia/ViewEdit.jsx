...
import { getAllSanPhamChiTietBySanPhamIdApi, getSanPhamChiTietByIdApi } from '../../../api/SanPhamChiTietAPI';
import { createSaleApi, getAllTenChienDichApi, updateSaleApi } from '../../../api/SaleApi';
import { useEffect } from 'react';
import { useNavigate } from 'react-router-dom';
import { getSaleByIdApi } from '../../../api/SaleApi';
...
const ViewEditDotGiamGia = () => {
    ...
    const [tenChienDichById, setTenChienDichById] = useState('');
        ...
        if (id) {
            const fetchSale = async () => {
                try {
                    ...
                    if (res && res.data) {
                        const data = res.data;
                        setTenChienDichById(data.tenChienDich);
                        console.log(data);
                        form.setFieldsValue({
                            tenChienDich: data.tenChienDich,
                        ...
                        console.log(form.getFieldValue('trangThai'));
                        const idSpcts = data.saleCts.map((item) => item.idSanPhamCt);

                        if (idSpcts.length > 0) {
                            const allProductDetails = await Promise.all(
                                idSpcts.map(async (id) => {
                                    const res = await getSanPhamChiTietByIdApi(id);
                                    return res.data;
                                })
                            );

                            const idSp = [... new Set(allProductDetails.map((item) => item.id_sanPham))];

                            const allProductDetailsSP = await Promise.all(
                                idSp.map(async (id) => {
                                    const res = await getAllSanPhamChiTietBySanPhamIdApi(id);
                                    return res.data;
                                }
                                )
                            );
                            console.log(allProductDetailsSP);
                            const allProductDetailsSPWithKey = allProductDetailsSP.map((item) => item.map((i) => ({
                                ...i,
                                key: i.id,
                            })));
                            const flattenProductDetailsSP = allProductDetailsSPWithKey.flat();
                            setTableDataSPCT(flattenProductDetailsSP);
                            setSelectedRowKeys(idSp);
                            setSelectedRowsKeysSPCT(idSpcts);
    ...
    const rowSelectionSPCT = {
        selectedRowKeys: selectedRowsKeysSPCT,
    ...
    const fetchTenChienDich = useCallback(async () => {
        try {
            ...
            if (res) {
                const filterTenChienDich = res.filter(
                    (item) => item !== tenChienDichById
                );

                console.log("Filtered results:", filterTenChienDich);
                setExitsTenChienDich(filterTenChienDich);
            }
    ...
    const handleSubmit = async () => {
        try {
            ...
            const res = await updateSaleApi(id, requestData);
            if (res && res.data) {
                notification.success({
                    message: "Success",
                    description: "Sửa đợt giảm giá thành công",
                });
                navigate('/admin/sale');
            }
    ...
    return (
        ...
        <>
            ...
            <Breadcrumb className="text-xl font-semibold mb-2">
                ...
                <Breadcrumb.Item>Sửa đợt giảm giá</Breadcrumb.Item>
            </Breadcrumb>
                            ...
                            <Form.Item name="trangThai" valuePropName="checked">
                                <Switch
                                    ...
                                    onChange={(checked) => {
                                        ...
                                        form.setFieldsValue({ trangThai: checked ? 1 : 0 });
                                    }}
                                />
                            </Form.Item>
                            {/* <Form.Item name="trangThai" valuePropName="checked" >
...
export default ViewEditDotGiamGia;